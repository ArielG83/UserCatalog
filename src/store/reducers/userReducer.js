...
const parseUser = (newUser, selectUser) => {
  ...
  return {
    ...
    matchScore: isEmpty(selectUser)
      ? -1
      : matchScore({ fullName, age: calculateAge(birthDate) }, selectUser),
  };
};
...
const parseRandomUser = (originalUser, selectUser) => {
  ...
  return {
    ...
    matchScore: isEmpty(selectUser)
      ? -1
      : matchScore({ age, fullName: `${first} ${last}` }, selectUser),
  };
};
  ...
  switch (type) {
    ...
    case USER.SELECT_USER:
      ...
      const reCalculatedUsers = state.users.map((user) => {
        const newMatchScore =
          user.id !== selectUser.id ? matchScore(user, selectUser) : -1;
